...
/// Hard cap on the first-run download (~12 MB).
const DOWNLOAD_TIMEOUT: Duration = Duration::from_secs(10 * 60);

const CREATE_NO_WINDOW: u32 = 0x0800_0000;

/// Where the managed executable lives for this install.
...
/// (the download in flight, the update stamp).
...
/// Fetch the official release into a `.part` file, then rename it into
/// place. The .part indirection means a torn download never masquerades
...
/// limit — the same reason the download URL above goes through
/// `latest/download/`.
async fn latest_release_tag() -> Option<String> {
    ...
    // Comparing against a non-version would re-download the binary on every
    // single check.
...
/// The single-file release replaces itself in place.
